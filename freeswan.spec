...
Podstawowa idea IPSEC to zapewnienie funkcji bezpieczeństwa
(autentykacji i szyfrowania) na poziomie IP. Będzie wymagany do IP w
wersji 6 (znanego także jako IPng, IP następnej generacji) i jest
opcjonalny dla aktualnego IP, w wersji 4.

FreeS/WAN jest darmową implementacją protokołu IPSEC.

%if %{klips}
%package -n kernel-%{_kernel_ver}-ipsec
Summary: FreeS/WAN IPSec kernel module
Summary(pl.UTF-8):   Moduł IPSec do jądra
